...
interface SPKGenerateResponse {
  spk: string;
  fallback?: boolean;
}

interface SPKVerifyResponse {
  valid: boolean;
  message?: string;
  fallback?: boolean;
  reservation?: {
...
function log(type: 'info' | 'success' | 'error' | 'warn' | 'step', message: string): void {
  switch (type) {
    case 'info':
...
async function testGenerateSPK(): Promise<string> {
  ...
  try {
    ...
    const data = await response.json() as SPKGenerateResponse;
    ...
    log('error', `Failed to generate SPK: ${(error as Error).message}`);
    throw error;
  }
}

// Test the SPK verification endpoint
async function testVerifySPK(spk: string): Promise<SPKVerifyResponse> {
  ...
  try {
    ...
    const data = await response.json() as SPKVerifyResponse;
    ...
    log('error', `Failed to verify SPK: ${(error as Error).message}`);
    throw error;
  }
}

// Test an invalid SPK
async function testInvalidSPK(): Promise<any> {
  ...
  try {
    ...
    log('error', `Error testing invalid SPK: ${(error as Error).message}`);
    throw error;
  }
}
