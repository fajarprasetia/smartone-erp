...
export default function EditPendingOrderPage() {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        console.log("Fetching pending order with ID:", orderId);

        // Use the pending orders API endpoint
        ...
        if (!response.ok) {
          ...
          // For database errors or other server errors, try the regular endpoint as fallback
          if (response.status === 500) {
            console.log("Trying fallback to regular orders endpoint");
            const fallbackResponse = await fetch(`/api/orders/${orderId}`, {
              headers: {
                'Accept': 'application/json',
                'Cache-Control': 'no-cache',
              },
            ...
            if (!fallbackData || !fallbackData.id) {
              throw new Error("Received invalid order data from fallback endpoint");
            }
            
            console.log("Successfully retrieved order from fallback endpoint");
        ...
        toast.error(`Failed to load pending order: ${errorMessage}`);
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Edit Pending Order</h1>
        <Button 
          variant="outline" 
          onClick={() => router.push("/order")}
        >
